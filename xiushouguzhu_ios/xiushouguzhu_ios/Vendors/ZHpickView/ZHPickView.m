...
-(instancetype)initDatePickWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode dateFormat:(NSString *)df isHaveNavControler:(BOOL)isHaveNavControler{
    
    self=[super init];
    if (self) {
        _defaulDate=defaulDate;
        [self setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode dateFormat:df];
        [self setFrameWith:isHaveNavControler];
        _btnBackground = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
        _btnBackground.backgroundColor=[UIColor blackColor];
        _btnBackground.alpha=0;
        [_btnBackground addTarget:self action:@selector(remove) forControlEvents:UIControlEventTouchUpInside];
    }
    return self;
}
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode dateFormat:df{
    self.dateFormat=df;
...
//    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStylePlain target:self action:@selector(remove)];
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc]init];
...
-(void)remove{
    ...
    [UIView animateWithDuration:0.3
         animations:^{
             self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height+64, self.frame.size.width, self.frame.size.height);
             _btnBackground.alpha=0;

         }
         completion:^(BOOL finished){
             [_btnBackground removeFromSuperview];
             _btnBackground=nil;
...
-(void)show{
    [[UIApplication sharedApplication].keyWindow addSubview:_btnBackground];
    self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y+self.frame.size.height+64, self.frame.size.width, self.frame.size.height);
    [[UIApplication sharedApplication].keyWindow addSubview:self];
    [UIView beginAnimations:nil context:nil];
    [UIView setAnimationDuration:0.2];
    _btnBackground.alpha=0.6;
    self.frame=CGRectMake(self.frame.origin.x, self.frame.origin.y-self.frame.size.height-64, self.frame.size.width, self.frame.size.height);
    [UIView commitAnimations];
