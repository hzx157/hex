...
- (id)initWithStyle:(HZAreaPickerStyle)pickerStyle delegate:(id<HZAreaPickerDelegate>)delegate
{
    ...
    self.layer.cornerRadius = 30;//圆弧半径
    if (self) {
...
- (void)showInView:(UIView *) view
{
    ...
    btnBackground = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
    btnBackground.backgroundColor=[UIColor blackColor];
    btnBackground.alpha=0;
    [btnBackground addTarget:self action:@selector(cancelPicker) forControlEvents:UIControlEventTouchUpInside];
    [view addSubview:btnBackground];
    self.frame = CGRectMake(2, -self.frame.size.height, view.frame.size.width-4, self.frame.size.height);
    [view addSubview:self];
    self.alpha=0;
    [UIView animateWithDuration:0.3 animations:^{
        btnBackground.alpha=0.6;
        self.frame = CGRectMake(2, view.center.y - self.frame.size.height/2, view.frame.size.width-4, self.frame.size.height);
        self.alpha=0.7;
    }];
...
- (IBAction)Commit:(UIButton*)sender {
    if (sender.tag==0) {
        ...
        if ([self.delegate respondsToSelector:@selector(pickerDidCommitStatus:)]) {
            if ([self.locate.district isEqualToString:@""]) {
                [self.delegate pickerDidCommitStatus:[NSString stringWithFormat:@"%@市 %@区",self.locate.state,self.locate.city]];
            }else{
                [self.delegate pickerDidCommitStatus:[NSString stringWithFormat:@"%@省 %@市 %@",self.locate.state,self.locate.city,self.locate.district]];
            }
