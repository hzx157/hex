...
-(id)initWithImageName:(NSString *)imageName title:(NSString *)titleStr x:(float)xPoint y:(float)yPoint height:(float)height
{
    ...
    if (self) {
        ...
        if(titleStr&&![titleStr isEqualToString:@""]){
            ...
            //设置背景条
            UIView *titleBack = [[UIView alloc]initWithFrame:CGRectMake(0, self.frame.size.height-30, width, 30)];
            titleBack.backgroundColor =[UIColor blackColor];
            titleBack.alpha=0.6;
            [self addSubview:titleBack];
            //设置标题文字
            UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, self.frame.size.height-30, width, 30)];
            titleLabel.text=titleStr;
            titleLabel.backgroundColor =[UIColor clearColor];
            titleLabel.textColor=[UIColor whiteColor];
            [self addSubview:titleLabel];
        }else{
            UrlImageView *imageView = [[UrlImageView alloc]initWithFrame:CGRectMake(0, 0, width, height)];
            //给定网络图片路径
            [imageView setImageFromUrl:YES withUrl:imageName];
            [self addSubview:imageView];
        }
